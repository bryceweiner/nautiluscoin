...
void Shutdown()
{
    ...
    RenameThread("nautiluscoin-shutoff");
...
bool AppInit(int argc, char* argv[])
{
    ...
    try
    {
        ...
        // Parameters
        //
        // If Qt is used, parameters/nautiluscoin.conf are parsed in qt/bitcoin.cpp's main()
        ...
        if (mapArgs.count("-?") || mapArgs.count("--help"))
        {
            ...
            std::string strUsage = _("Nautiluscoin version") + " " + FormatFullVersion() + "\n\n" +
                _("Usage:") + "\n" +
                  "  nautiluscoind [options]                     " + "\n" +
                  "  nautiluscoind [options] <command> [params]  " + _("Send command to -server or nautiluscoind") + "\n" +
...
std::string HelpMessage()
{
    string strUsage = _("Options:") + "\n" +
        "  -?                     " + _("This help message") + "\n" +
        "  -conf=<file>           " + _("Specify configuration file (default: nautiluscoin.conf)") + "\n" +
        "  -pid=<file>            " + _("Specify pid file (default: nautiluscoind.pid)") + "\n" +
        ...
        "\n" + _("SSL options: (see the Nautiluscoin Wiki for SSL setup instructions)") + "\n" +
...
{
    RenameThread("nautiluscoin-loadblk");
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Nautiluscoin is probably already running."), strDataDir.c_str()));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    printf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    printf("Nautiluscoin version %s (%s)\n", FormatFullVersion().c_str(), CLIENT_DATE.c_str());
    ...
    if (nLoadWalletRet != DB_LOAD_OK)
    {
        ...
        else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
        {
            ...
            strErrors << _("Error loading wallet.dat: Wallet requires newer version of Nautiluscoin") << "\n";
        else if (nLoadWalletRet == DB_NEED_REWRITE)
        {
            strErrors << _("Wallet needed to be rewritten: restart Nautiluscoin to complete") << "\n";
