...
{
    QString ret = QString("nautiluscoin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non PHI unit input in lnReqAmount, we generate the URI with PHI as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::PHI, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
