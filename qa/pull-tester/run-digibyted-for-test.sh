...
#Install a watchdog.
(sleep 10 && kill -0 $WAITER 2>/dev/null && kill -9 $NAUTILUSCOIND $$)&
wait $WAITER
...
(sleep 15 && kill -0 $NAUTILUSCOIND 2>/dev/null && kill -9 $NAUTILUSCOIND $$)&
kill $NAUTILUSCOIND && wait $NAUTILUSCOIND
