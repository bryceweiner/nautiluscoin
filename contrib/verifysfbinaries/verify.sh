...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'nautiluscoin-',
...
if [ $? -ne 0 ]; then
   ...
   echo "[nautiluscoin-]<version>-[rc[0-9]] (example: nautiluscoin-0.7.1-rc1)"
   echo "wget output:"
